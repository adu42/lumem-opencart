...
class AmazonusProduct extends Controller{
					...
					foreach($template['fields'] as $key => $field) {
						if ($field['accepted']['type'] == 'image') {
							if (empty($field['value'])) {
								...
								$template['fields'][$key]['thumb'] = $this->model_tool_image->resize(str_replace(HTTPS_CATALOG . DIR_IMAGE_WEB_PATH, '', $field['value']), 100, 100);
							}
						}
					}
	...
	private function fillDefaultValues($product_id, $fields_array, $var = '') {
		...
		$product_info['image'] = HTTPS_CATALOG . DIR_IMAGE_WEB_PATH . $product_info['image'];
		...
		$weight_use App\System\Engine\Controller;
use App\System\Library\Log;
class = $this->model_localisation_weight_class->getWeightClass($product_info['weight_class_id']);
		...
		foreach($product_images as $product_image) {
			$defaults['pt' . $image_index] = HTTPS_CATALOG . DIR_IMAGE_WEB_PATH . $product_image['image'];
			$image_index ++;
		}
		...
		if ($var !== '' && $this->openbay->addonLoad('openstock')) {
			...
			if ($option != null) {
				...
				if (!empty($option['image'])) {
					$defaults['mainimage'] = HTTPS_CATALOG . DIR_IMAGE_WEB_PATH . $option['image'];
				}
			}
		}
