<?php namespace App\Http\Controllers\Catalog\Event;
...
class Theme extends Controller {
	...
	public function override(&$route, &$args, &$template) {
		...
		} elseif (is_file(DIR_TEMPLATE . $theme . '/' . $route . '.twig')) {
			$this->config->set('template_directory', $theme . '/');
		} elseif (is_file(DIR_TEMPLATE . 'default/' . $route . '.twig')) {
			$this->config->set('template_directory', 'default/');
		}
	}
}