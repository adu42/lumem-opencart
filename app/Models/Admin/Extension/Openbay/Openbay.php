<?php namespace App\Models\Admin\Extension\Openbay;
...
class ModelExtensionOpenBayOpenbay extends Model {
	...
	public function updateTest() {
		...
		try {
			$output  = '<?php namespace App\Models\Admin\Extension\Openbay;' . "\n";
			$output  .= '$test = \'12345\';' . "\n";
			$output  .= 'echo $test;' . "\n";
    ...
    public function storeImage($filename, $width, $height, $sub_directory = '') {
		...
		if ($this->request->server['HTTPS']) {
			return HTTPS_CATALOG . DIR_IMAGE_WEB_PATH . $image_new;
		} else {
			return HTTP_CATALOG . DIR_IMAGE_WEB_PATH . $image_new;
		}
	}
}
