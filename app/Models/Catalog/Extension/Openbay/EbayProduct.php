	...
	public function importItems($data) {
		...
		foreach ($new_data_1 as $item) {
			if (!in_array($item['ItemID'], $current)) {
				...
				//get the length use App\System\Engine\Model;
class id
				$length_class_id = 1;
				...
				//get the weight use App\System\Engine\Model;
class id
				$weight_class_id = 1;
	...
	public function resize($filename, $width, $height, $type = "") {
		...
		if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1'))) {
			return $this->config->get('config_ssl') . DIR_IMAGE_WEB_PATH . $image_new;
		} else {
			return $this->config->get('config_url') . DIR_IMAGE_WEB_PATH . $image_new;
		}
	}
